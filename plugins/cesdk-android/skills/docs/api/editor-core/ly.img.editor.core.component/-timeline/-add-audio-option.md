...
```kotlin
@Stable
data class AddAudioOption(val scope: Timeline.ItemScope, val id: EditorComponentId, val modifier: Modifier, val visible: Boolean, val enterTransition: EnterTransition, val exitTransition: ExitTransition, val decoration: ScopedDecoration<Timeline.ItemScope>, val onClick: Timeline.ItemScope.() -> Unit, val icon: @Composable Timeline.ItemScope.() -> Unit?, val text: @Composable Timeline.ItemScope.() -> Unit?, val enabled: Boolean) : EditorComponent<Timeline.ItemScope>
```
