...
See [Pages](./pages.md) for details.
...
See [Design Units](./design-units.md) for details.
...
On Android, choose the starting configuration when creating the scene: `engine.scene.create()` for a static design layout, or `engine.scene.createForVideo()` for video editing. See [Scenes](./scenes.md) for details.
...
In *Headless Mode*, you work directly with *Engine* APIs without the visual editor. See [Headless Mode](./headless-mode.md) for setup.
...
On Android, block lifecycle subscriptions are exposed as Kotlin `Flow`s. Cancel the coroutine collecting the `Flow` when you no longer need notifications. See [Events](./events.md) for details.
