...
> **Resources:**
>
> - [Download examples](https://img.ly/release/v1.82.1/apps/documentation/downloads/download-maven-version.sh)
...
```bash
./download-maven-version.sh --dry-run 1.82.1
```
...
```bash
./download-maven-version.sh 1.82.1 ./maven-offline
```

The script downloads every file in an exact `1.82.1` version directory and keeps the Maven layout. The generated local repository starts at `maven-offline/maven` and contains paths such as `ly/img/editor-core/1.82.1/`.
