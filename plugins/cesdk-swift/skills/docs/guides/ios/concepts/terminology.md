...
**Navigation:** [Concepts](../concepts.md) > [Terminology](./terminology.md)
...
All operations—creating scenes, manipulating blocks, rendering, and exporting—go through the *Engine*. In Swift, this is the `Engine` class from `IMGLYEngine`. Initialize it once and use it throughout your application's lifecycle.
...
See [Scenes](./scenes.md) for details.

### Page

*Pages* are containers within a *Scene* that hold content *Blocks* (see below) and define working area dimensions.

In *Design Mode*, pages are individual artboards. In *Video Mode*, pages are timeline compositions where *Blocks* are arranged across time.
...
See [Blocks](./blocks.md) for details.
...
See the Color Fills, Gradient Fills, Image Fills, and Video Fills guides.
...
Like *Fills*, *Shapes* are created separately and attached to *Blocks*.
...
The order determines how multiple effects attached to a single block interact.
...
- `//ly.img.ubq/zone` — Forbidden zone, an editor-only region content must stay out of
...
Access *Properties* using type-specific getter and setter methods. Each *Block* type exposes different properties, which you can discover programmatically. See [Blocks](./blocks.md) for details.
...
Loaded data from an *Asset* URI. When you reference an image or video URL in a *Block*, the *Engine* fetches and caches the *Resource*. *Resources* include binary data and metadata for rendering. See [Resources](./resources.md) for details.
...
Create a *Buffer*, write data to it, and reference it by URI in *Block* properties. *Buffer* data is not serialized with scenes and changes cannot be undone. See [Buffers](./buffers.md) for details.
...
Choose the mode when creating a scene—it affects which properties and operations are available. See [Scenes](./scenes.md) for details.
...
In *Headless Mode*, you work directly with *Engine* APIs without the visual editor.
...
Subscriptions return a cancellable token to stop listening when you no longer need notifications. See [Events](./events.md) for details.
