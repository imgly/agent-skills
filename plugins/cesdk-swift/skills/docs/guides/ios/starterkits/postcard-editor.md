...
```swift file=@cesdk_swift_examples/starter-kits/starter-kit-postcard/StarterKit/PostcardEditorStarterKit.swift reference-only
import IMGLYEditor
import IMGLYEngine
import SwiftUI
...
```swift file=@cesdk_swift_examples/starter-kits/starter-kit-postcard/StarterKit/callbacks/OnCreate+Postcard.swift reference-only
import Foundation
import IMGLYEditor
import IMGLYEngine
...
```swift file=@cesdk_swift_examples/starter-kits/starter-kit-postcard/StarterKit/callbacks/OnExport+Postcard.swift reference-only
import IMGLYEditor
import IMGLYEngine
...
Let users personalize postcards with templates, style presets, and print-ready exports. Runs entirely on the mobile device with no server dependencies.

![Postcard Editor starter kit screenshot](https://img.ly/docs/cesdk/ios/starterkits/postcard-editor-03mijr/assets/ios.hero.webp)

> **Reading time:** 10 minutes
>
> **Resources:**
>
> - [Download examples](https://github.com/imgly/starterkit-postcard-editor-ios/archive/refs/heads/v1.83.0-nightly.20260903.zip)
>
> - [View source on GitHub](https://github.com/imgly/starterkit-postcard-editor-ios/tree/v1.83.0-nightly.20260903)
...
<Tabs syncKey="project-type">
  <TabItem label="New Project">
    ...
    ```bash
    git clone -b v1.83.0-nightly.20260903 https://github.com/imgly/starterkit-postcard-editor-ios.git
    ...
    1. Open `starterkit-postcard-editor-ios.xcodeproj` in Xcode
    2. Select your target device or simulator
    3. Press **⌘R** to build and run

    The sample app shows a "Launch Editor" button. Tapping it presents the postcard editor:
...
```text
starter-kit/
├── PostcardEditorStarterKit.swift      # SwiftUI view that launches the editor
├── PostcardEditorConfiguration.swift   # Editor configuration (callbacks + UI components)
├── callbacks/
│   ├── OnCreate+Postcard.swift          # Editor initialization logic
│   ├── OnExport+Postcard.swift          # Export flow and handling (PDF)
│   └── OnChanged+Postcard.swift         # Page and view mode change handling
└── components/
    ├── CanvasMenu+Postcard.swift        # Canvas menu configuration
    ├── Dock+Postcard.swift              # Dock configuration (greeting tools)
    ├── InspectorBar+Postcard.swift      # Inspector bar configuration
    └── NavigationBar+Postcard.swift     # Navigation bar configuration (page navigation)
```
...
The entry point is `PostcardEditorStarterKit.swift`, which creates the `Editor` view and applies the configuration:
...
You can configure the editor based on your business logic — for example, loading a scene from a previous editing session or displaying different UI for different users. Add properties to `PostcardEditorStarterKit` and use them in the `EditorConfiguration.onCreate` callback to customize scene loading or other behavior.

## Set Up a Scene

The scene setup logic is located in `OnCreate+Postcard.swift` as part of the `defaultCreateScene` callback:
...
The asset source setup is located in `OnCreate+Postcard.swift` as part of the `defaultLoadAssetSources` callback. Enable or disable individual sources:
...
Export handling logic is located in `OnExport+Postcard.swift` as part of the `onExport` callback.
...
- `CanvasMenu+Postcard.swift` — see [Canvas Menu](../user-interface/customization/canvas-menu.md) for full configuration options
- `Dock+Postcard.swift` — see [Dock](../user-interface/customization/dock.md) for full configuration options
- `InspectorBar+Postcard.swift` — see [Inspector Bar](../user-interface/customization/inspector-bar.md) for full configuration options
- `NavigationBar+Postcard.swift` — see [Navigation Bar](../user-interface/customization/navigation-bar.md) for full configuration options
