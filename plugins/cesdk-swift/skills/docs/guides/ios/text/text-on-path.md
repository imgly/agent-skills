...
![The words TEXT ON A PATH curving along the left arc of a circular path, with vertical alignment Center and a slight offset along the path](https://img.ly/docs/cesdk/ios/text/text-on-path-e3b8a2/assets/swift-based.hero.webp)
...
- [Disable or Enable Features](../user-interface/customization/disable-or-enable.md) — on iOS, gate the Path button and other editor components with the `isVisible`/`isEnabled` pattern
