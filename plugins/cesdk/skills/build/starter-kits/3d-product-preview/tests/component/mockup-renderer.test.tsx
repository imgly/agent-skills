...
import { DEFAULT_RENDER_DEBOUNCE_MS } from '../../src/app/constants';
...
describe('P3D-C3 rendering a product', () => {
  it('renders from the product scene URL and publishes the result', async () => {
    const { result } = setup();

    await act(async () => {
      await result.current.renderMockupForProduct('apparel');
    });
  ...
  it('renders from the scene string once the mockup has been edited', async () => {
    ...
    await act(async () => {
      await result.current.updateMockupScene('<edited/>', 'apparel');
    });
  ...
  it('goes back to the product scene URL after a reset', async () => {
    ...
    await act(async () => {
      await result.current.renderMockupForProduct('cap');
    });
...
describe('P3D-C4 renders that overlap', () => {
  it('queues the second request and runs it once the first one is done', async () => {
    ...
    await act(async () => {
      first = result.current.renderMockupForProduct('apparel');
    });
    await act(async () => {
      await result.current.renderMockupForProduct('cap');
    });
...
describe('P3D-C5 auto-refresh on a design change', () => {
  ...
  it('queues a design change that arrives while a render is in flight', async () => {
    ...
    await act(async () => {
      second = result.current.renderMockupForProduct('apparel');
    });
    act(() => historyListener?.());
    expect(mocks.renderMockup).toHaveBeenCalledTimes(2);
