...
const create = vi.fn();
const initDesignEditor = vi.fn(async () => undefined);
const reportDemoPhase = vi.fn();

vi.mock('@cesdk/cesdk-js', () => ({ default: { create } }));
vi.mock('../../src/imgly', () => ({ initDesignEditor }));
...
const SCENE = '/assets/4-5-marketing-ad/scene.scene';

describe('DE-U5 src/index.ts', () => {
  beforeEach(() => {
    vi.resetModules();
    vi.unstubAllEnvs();
    create.mockReset();
    initDesignEditor.mockClear();
  ...
  it('loads the scene from the configured base URL', async () => {
    ...
    expect(config.userId).toBe('starterkit-design-editor-user');
    expect(initDesignEditor).toHaveBeenCalledWith(editor);
  ...
  it('reports a failed start-up instead of leaving an unhandled rejection', async () => {
    ...
    expect(initDesignEditor).not.toHaveBeenCalled();
