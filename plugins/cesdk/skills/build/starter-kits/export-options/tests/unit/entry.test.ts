...
const create = vi.fn();
const initExportOptionsEditor = vi.fn(async () => undefined);
const reportDemoPhase = vi.fn();

vi.mock('@cesdk/cesdk-js', () => ({ default: { create } }));
vi.mock('../../src/imgly', () => ({ initExportOptionsEditor }));
...
describe('EO-U10 src/index.ts', () => {
  beforeEach(() => {
    vi.resetModules();
    create.mockReset();
    initExportOptionsEditor.mockClear();
  ...
  it('configures the editor and loads the demo scene', async () => {
    ...
    expect(config.userId).toBe('starterkit-export-options-user');
    expect(initExportOptionsEditor).toHaveBeenCalledWith(editor);
  ...
  it('reports a failed start-up instead of leaving an unhandled rejection', async () => {
    ...
    expect(initExportOptionsEditor).not.toHaveBeenCalled();
