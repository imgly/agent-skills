...
import { initExportOptionsEditor } from '../../src/imgly';
...
describe('EO-U12 initExportOptionsEditor', () => {
  async function run() {
    const spy = createApiSpy<CreativeEditorSDK>();
    await initExportOptionsEditor(spy.api);
    return spy;
  }
  ...
  it('adds the kit configuration first, then the asset sources', async () => {
    ...
    expect(plugins.slice(0, 1).map(({ name }) => name)).toEqual([
      'cesdk-design-editor'
    ]);
    expect(plugins.slice(1).map(({ source }) => source)).toContain(
      'ImageColorsAssetSource'
    );
  });
  ...
  it('registers every asset source at once, not one after another', async () => {
    ...
    const done = initExportOptionsEditor(api);
