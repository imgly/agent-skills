...
const create = vi.fn();
const initGettyImagesEditor = vi.fn(async () => undefined);
const reportDemoPhase = vi.fn();

vi.mock('@cesdk/cesdk-js', () => ({ default: { create } }));
vi.mock('../../src/imgly', () => ({ initGettyImagesEditor }));
...
describe('GET-U21 src/index.ts', () => {
  beforeEach(() => {
    vi.resetModules();
    create.mockReset();
    initGettyImagesEditor.mockClear();
  ...
  it('creates the editor, initialises Getty Images and loads the demo scene', async () => {
    ...
    expect(config.userId).toBe('starterkit-getty-asset-source-user');
    expect(initGettyImagesEditor).toHaveBeenCalledWith(editor, {
      gettyProxyUrl: undefined
    });
    expect(editor.load).toHaveBeenCalledWith(
      'http://localhost/assets/getty-images.scene'
    );
  ...
  it('reports a failed start-up instead of leaving an unhandled rejection', async () => {
    ...
    expect(initGettyImagesEditor).not.toHaveBeenCalled();
