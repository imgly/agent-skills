...
import { initGettyImagesEditor } from '../../src/imgly';
import { GettyImagesAssetSourcePlugin } from '../../src/imgly/plugins/getty-images';
...
describe('GET-U6 initGettyImagesEditor', () => {
  it('passes the proxy URL through to the plugin', async () => {
    const spy = createApiSpy<CreativeEditorSDK>();

    await initGettyImagesEditor(spy.api, {
      gettyProxyUrl: 'https://getty-proxy.test/api'
    });
    ...
    const plugin = spy
      ...
      .map(({ args }) => args[0])
      .find(
        (candidate): candidate is GettyImagesAssetSourcePlugin =>
          candidate instanceof GettyImagesAssetSourcePlugin
      );
    expect(plugin).toBeDefined();
    expect(pluginOptions(plugin!).proxyUrl).toBe(
      'https://getty-proxy.test/api'
    );
  });

  it('adds the asset sources in one concurrent batch, between the config plugin and Getty Images', async () => {
    const editor = pausingEditor();

    const done = initGettyImagesEditor(editor.cesdk);
    ...
    await editor.releaseAll();
    expect(editor.added.at(-1)).toBe('cesdk-getty-images-asset-source');
...
function pluginOptions(plugin: GettyImagesAssetSourcePlugin): {
  proxyUrl?: string;
} {
  return (plugin as unknown as { options: { proxyUrl?: string } }).options;
}
...
describe('GET-U6 GettyImagesAssetSourcePlugin', () => {
  async function initialise(options?: { proxyUrl?: string }) {
    const spy = createApiSpy<CreativeEditorSDK>();
    setupDock(spy.api);
    const dock = (
    ...
    await new GettyImagesAssetSourcePlugin(options).initialize({
      cesdk: editor.cesdk
    } as never);
    return editor;
  }

  it('replaces the Images dock entry with Getty Images', async () => {
    const editor = await initialise({
      proxyUrl: 'https://getty-proxy.test/api'
    });

    const entry = editor
      .dock()
      .find((item) => item.key === 'gettyImagesImageAssets');
  ...
  it('offers Getty Images for an image fill and the defaults otherwise', async () => {
    const editor = await initialise({
      proxyUrl: 'https://getty-proxy.test/api'
    });
    ...
    ).toEqual(['gettyImagesImageAssets']);
    expect(
      pick({
  ...
  it('falls back to an empty proxy URL when none is given', async () => {
    const editor = await initialise();
