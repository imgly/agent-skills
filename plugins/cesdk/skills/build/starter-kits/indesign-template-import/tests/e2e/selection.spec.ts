...
test.describe('File selection', () => {
  test('IDML-01 the kit starts on the selection screen with no engine', async ({
    page
  }) => {
    const app = new KitApp(page);
    await app.open();

    await expect(page.getByText('Supports .idml Format')).toBeVisible();
  ...
  test('IDML-04 a dropped file is imported', async ({ page }) => {
    const app = new KitApp(page);
    await app.open();
  ...
  test('IDML-05 a dropped file of another type is not imported', async ({
    page
  }) => {
    const app = new KitApp(page);
    await app.open();

    await app.drop('not-a-idml.txt', Buffer.from('x'.repeat(1024)));
  ...
  test('IDML-05b a rejected drop says why', async ({ page }) => {
    const app = new KitApp(page);
    await app.open();
    ...
    await expect(
      page.getByText(/\.idml/i).and(page.getByRole('alert'))
    ).toBeVisible();
  });
});
