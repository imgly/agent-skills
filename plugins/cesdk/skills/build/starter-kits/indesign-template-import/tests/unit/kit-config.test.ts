...
import { initInDesignTemplateImportEditor } from '../../src/imgly';
...
describe('IDML-U8 initInDesignTemplateImportEditor', () => {
  const spy = createApiSpy<CreativeEditorSDK>();
  let added: AddedPlugin[] = [];

  beforeAll(async () => {
    await initInDesignTemplateImportEditor(spy.api);
  ...
  it('registers every asset source in one concurrent batch', async () => {
    ...
    const configured = initInDesignTemplateImportEditor(cesdk);
...
describe('IDML-U15 the video timeline the kit ships is inert', () => {
