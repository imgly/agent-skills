...
- Browser: Chrome, headless, 1400 × 900. Requests to `cdn.img.ly` fail the test, except the typeface fonts the three demo scenes store as absolute URIs (`cdn.img.ly/assets/v3/ly.img.typeface/fonts/`). That one allowlist entry is in `tests/playwright.config.ts`; everything else, including all demo images, is served locally.
