...
describe('PDF-U6 the archive download name', () => {
  it.each([
    ['postcard.pdf', 'postcard.imgly'],
    ['A.PDF', 'A.imgly'],
...
describe('the result screen shows both files side by side', () => {
  it('names the two columns and offers Edit, Download and New File', () => {
    renderResult();
    expect(screen.getByRole('heading', { name: 'PDF File' })).toBeDefined();
    expect(
  ...
  it('shows the original preview for a pre-loaded example', () => {
    ...
    expect(screen.getByAltText('Original PDF File')).toHaveProperty(
      'src',
      EXAMPLE.previewUrl
    );
  });
  ...
  it('says there is no preview for an uploaded file', () => {
    renderResult('uploaded.pdf', UPLOADED);
    expect(screen.getByText('No Preview Available')).toBeDefined();
    expect(screen.queryByAltText('Original PDF File')).toBeNull();
    expect(screen.getByText(/uploaded\.pdf/)).toBeDefined();
  });
  ...
  it('names the file generically when there is nothing to name', () => {
    renderResult('uploaded.pdf', null);
