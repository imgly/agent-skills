...
function renderZone() {
  ...
  return screen.getByLabelText(/Upload PDF File/);
}
...
describe('PDF-U1 the upload zone filters what it forwards', () => {
  it.each(['a.pdf', 'a.PDF'])('forwards a dropped %s', (name) => {
  ...
  it.each(['a.txt', 'a.png', 'noextension'])('ignores a dropped %s', (name) => {
    const input = renderZone();
    fireEvent.drop(input, { dataTransfer: { files: [file(name)] } });
  ...
  it('keeps the zone highlighted while the pointer moves over it', () => {
    ...
    fireEvent.dragOver(input, { dataTransfer: { files: [file('a.pdf')] } });
  ...
  it('highlights the zone while a file is dragged over it', () => {
    ...
    fireEvent.dragEnter(input, { dataTransfer: { files: [file('a.pdf')] } });
    expect(zone.className).toContain('dragging');

    fireEvent.dragLeave(input, { dataTransfer: { files: [file('a.pdf')] } });
    expect(zone.className).not.toContain('dragging');
  });
