...
test.describe('File selection', () => {
  test('PDF-01 the kit starts on the selection screen with no engine', async ({
    page
  }) => {
    const app = new KitApp(page);
    await app.open();

    await expect(page.getByText('Supports .pdf Format')).toBeVisible();
  ...
  test('PDF-04 a dropped file is imported', async ({ page }) => {
    const app = new KitApp(page);
    await app.open();
  ...
  test('PDF-05 a dropped file of another type is not imported', async ({
    page
  }) => {
    const app = new KitApp(page);
    await app.open();

    await app.drop('not-a-pdf.txt', Buffer.from('x'.repeat(1024)));
  ...
  test('PDF-05b a rejected drop says why', async ({ page }) => {
    const app = new KitApp(page);
    await app.open();
    ...
    await expect(
      page.getByText(/\.pdf/i).and(page.getByRole('alert'))
    ).toBeVisible();
  });
});
