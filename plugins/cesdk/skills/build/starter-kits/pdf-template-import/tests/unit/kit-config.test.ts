...
import { initPdfTemplateImportEditor } from '../../src/imgly';
...
describe('PDF-U8 initPdfTemplateImportEditor', () => {
  const spy = createApiSpy<CreativeEditorSDK>();
  let added: AddedPlugin[] = [];

  beforeAll(async () => {
    await initPdfTemplateImportEditor(spy.api);
  ...
  it.each([
    ...
    ['DemoAssetSources', ['ly.img.templates.*', 'ly.img.image.*']]
  ...
  it('leaves the other asset sources unrestricted', () => {
    ...
    expect(restricted).toEqual(['UploadAssetSources', 'DemoAssetSources']);
  });
  ...
  it('registers every asset source in one concurrent batch', async () => {
    ...
    const configured = initPdfTemplateImportEditor(cesdk);
...
describe('PDF-U15 the video timeline the kit ships is inert', () => {
