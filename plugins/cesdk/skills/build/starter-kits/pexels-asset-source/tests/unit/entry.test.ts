...
const create = vi.fn();
const initPexelsImageEditor = vi.fn(async () => undefined);
const reportDemoPhase = vi.fn();

vi.mock('@cesdk/cesdk-js', () => ({ default: { create } }));
vi.mock('../../src/imgly', () => ({ initPexelsImageEditor }));
...
describe('PEX-U21 src/index.ts', () => {
  beforeEach(() => {
    vi.resetModules();
    create.mockReset();
    initPexelsImageEditor.mockClear();
  ...
  it('creates the editor, initialises Pexels and loads the demo scene', async () => {
    ...
    expect(config.userId).toBe('starterkit-pexels-asset-source-user');
    expect(initPexelsImageEditor).toHaveBeenCalledWith(editor, {
      pexelsApiKey: undefined
    });
    expect(editor.load).toHaveBeenCalledWith(
      'http://localhost/assets/pexels.scene'
    );
  ...
  it('reports a failed start-up instead of leaving an unhandled rejection', async () => {
    ...
    expect(initPexelsImageEditor).not.toHaveBeenCalled();
