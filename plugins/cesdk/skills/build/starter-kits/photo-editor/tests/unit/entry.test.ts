...
const create = vi.fn();
const initPhotoEditor = vi.fn(async () => undefined);
const reportDemoPhase = vi.fn();

vi.mock('@cesdk/cesdk-js', () => ({ default: { create } }));
vi.mock('../../src/imgly', () => ({ initPhotoEditor }));
...
const SCENE = '/assets/16-9-fashion-ad/scene.scene';

describe('PE-U4 src/index.ts', () => {
  beforeEach(() => {
    vi.resetModules();
    vi.unstubAllEnvs();
    create.mockReset();
    initPhotoEditor.mockClear();
  ...
  it('loads the scene from the configured base URL', async () => {
    ...
    expect(config.userId).toBe('starterkit-photo-editor-user');
    expect(initPhotoEditor).toHaveBeenCalledWith(editor);
  ...
  it('reports a failed start-up instead of leaving an unhandled rejection', async () => {
    ...
    expect(initPhotoEditor).not.toHaveBeenCalled();
