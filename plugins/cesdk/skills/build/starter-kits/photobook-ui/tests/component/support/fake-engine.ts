...
export const PINK = { r: 220 / 255, g: 24 / 255, b: 118 / 255, a: 1 };
...
export function imageAsset(id: string) {
  return {
    id,
    context: { sourceId: 'ly.img.image.upload' },
...
export function createFakeEngine({
  ...
  const blocks = new Map<number, Block>([
    [SCENE, block({ type: '//ly.img.ubq/scene', children: [STACK] })],
    [
      STACK,
      block({
        type: '//ly.img.ubq/stack',
        parent: SCENE,
    ...
    [
      PAGE_B,
      block({ type: '//ly.img.ubq/page', parent: STACK, fill: PAGE_B_FILL })
    ],
    [
      TEXT_BLOCK,
      block({
  ...
  const assets = new Map<string, unknown[]>([
    ...
    ['ly.img.vector.shape', [imageAsset('shape-1')]],
    ['ly.img.sticker', [imageAsset('sticker-1')]],
    ['ly.img.layouts', [imageAsset('layout-1')]],
  ...
  const engine = {
    ...
    block: {
      ...
      insertChild: vi.fn((parent: number, child: number, index: number) => {
        const siblings = get(parent).children.filter((id) => id !== child);
      ...
      getSourceSet: vi.fn(() => []),
      setSourceSet: vi.fn(),
      supportsPlaceholderBehavior: vi.fn(() => false),
      isPlaceholderBehaviorEnabled: vi.fn(() => false),
      setPlaceholderEnabled: vi.fn(),
      isPlaceholderControlsOverlayEnabled: vi.fn(() => false),
      ...
      loadFromString: vi.fn(async () => {
        const id = ++nextId;
        blocks.set(id, block({ type: '//ly.img.ubq/page' }));
        return [id];
      }),
      export: vi.fn(async () => new Blob(['page'], { type: 'image/jpeg' })),
