...
vi.mock('../../src/imgly', () => ({
  initPlaceholdersCreatorEditor: (cesdk: CreativeEditorSDK) =>
    initCreator(cesdk),
  initPlaceholdersAdopterEditor: (cesdk: CreativeEditorSDK) =>
    initAdopter(cesdk)
}));

const reportDemoPhase = vi.fn();
const reportDemoLoadingState = vi.fn();
...
let editorProps: { onLoadingStateChange?: unknown } = {};
...
function CreativeEditorStub(props: {
...
describe('PLC-C1 the first mount', () => {
  it('loads the scene from the URL and auto-fits the page', async () => {
    ...
    expect(editorProps.onLoadingStateChange).toBe(reportDemoLoadingState);
  });
...
describe('PLC-C2 switching role', () => {
