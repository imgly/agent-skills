...
describe('PH-U2 the feature difference between the two roles (Qase 4685)', () => {
  const placeholders = (features: string[]) =>
    features.filter((id) => id.startsWith('ly.img.placeholder'));

  it('gives the placeholder options to the Creator config only', () => {
    expect(placeholders(enabledFeatures(setupAdvancedFeatures))).toEqual(
    ...
    expect(placeholders(enabledFeatures(setupDesignFeatures))).toEqual([]);
  });

  it('enables exactly the features each config lists', () => {
    expect(enabledFeatures(setupAdvancedFeatures)).toEqual([
      'ly.img.adjustment',
      'ly.img.blendMode',
      'ly.img.blur',
      'ly.img.canvas.bar',
    ...
    expect(enabledFeatures(setupDesignFeatures)).toEqual([
      'ly.img.adjustment',
      'ly.img.blendMode',
      'ly.img.blur',
      'ly.img.canvas.bar',
  ...
  it('gives both roles the same design editing base', () => {
    const advanced = enabledFeatures(setupAdvancedFeatures);
    const design = enabledFeatures(setupDesignFeatures);
    ...
    for (const feature of [
      ...
      expect(advanced).toContain(feature);
      expect(design).toContain(feature);
    }
  });

  it('keeps vector editing and rulers out of the Adopter config', () => {
    const design = enabledFeatures(setupDesignFeatures);
    expect(design).not.toContain('ly.img.vectorEdit');
    expect(design).not.toContain('ly.img.rulers');
  });
});

describe('PH-U3 the navigation bars', () => {
  it('gives both roles the same order and the same two export actions', () => {
    const advanced = navigationBarOrder(setupAdvancedNavigationBar);
    const design = navigationBarOrder(setupDesignNavigationBar);

    expect(design).toEqual(advanced);
    expect(advanced.at(-1)).toEqual({
  ...
  it('leaves Page Resize in the Adopter bar as well (known issue 6)', () => {
    expect(navigationBarOrder(setupDesignNavigationBar)).toContain(
      'ly.img.pageResize.navigationBar'
    );
  });
});

describe('PH-U4 the export actions', () => {
  it.each([
    ['Creator', setupAdvancedActions],
    ['Adopter', setupDesignActions]
  ])('registers the same five actions for %s', (_role, setup) => {
    const spy = createApiSpy<CreativeEditorSDK>();
    setup(spy.api);
