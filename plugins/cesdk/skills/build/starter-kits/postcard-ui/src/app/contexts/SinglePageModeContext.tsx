...
import { zoomToSelectedText } from '@/imgly/utils';
...
interface SinglePageModeContextType {
  ...
  // Imperatively zoom to a block and await the zoom; used to reveal the canvas
  // only after the initial zoom completes (instead of guessing with a timer).
  zoomToBlockId: (blockId: number) => Promise<void> | undefined;
...
export const SinglePageModeProvider = ({
  ...
  // State-independent zoom: callable right after setEnabled(true) without a
  // stale-closure on `enabled`/`currentPageBlockId`. Returns the zoom Promise.
  const zoomToBlockId = useCallback(
    (blockId: number) =>
      engine.block.isValid(blockId)
        ...
        : undefined,
    [engine, paddingLeft, paddingTop, paddingRight, paddingBottom]
  );
