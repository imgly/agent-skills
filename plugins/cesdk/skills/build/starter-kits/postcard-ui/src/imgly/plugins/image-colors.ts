 ...
 */
function createImageColorsSource(engine: CreativeEngine): AssetSource {
  return {
    id: ASSET_SOURCES.imageColors,
...
function addColorAsset(
  ...
  seen.add(key);
  collected.push({
    id: `${ASSET_SOURCES.imageColors}.${encodeURIComponent(group)}.${key}`,
