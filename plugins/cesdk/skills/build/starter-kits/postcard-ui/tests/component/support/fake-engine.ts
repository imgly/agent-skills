...
export const FRONT_PAGE = 10;
export const BACK_PAGE = 11;
export const TEXT_BLOCK = 20;
export const IMAGE_BLOCK = 21;
export const SHAPE_BLOCK = 22;
export const STICKER_BLOCK = 23;
export const ACCENT_BLOCK = 24;
export const BACKGROUND_BLOCK = 25;

const IMAGE_FILL = 210;
const SHAPE_FILL = 220;
const TEXT_FILL = 200;
const ACCENT_FILL = 240;
const BACKGROUND_FILL = 250;
const FRONT_FILL = 100;
const BACK_FILL = 110;
...
export interface FakeEngineOptions {
  ...
  cursorY?: number;
  /** Make `setSelected` publish a selection event, the way the engine does. */
  notifyOnSetSelected?: boolean;
}
...
export function createFakeEngine({
  ...
  cursorY = 0,
  notifyOnSetSelected = false
}: FakeEngineOptions = {}): FakeEngine {
  const blocks = new Map<number, Block>([
    [
      SCENE,
      block({ type: '//ly.img.ubq/scene', children: [FRONT_PAGE, BACK_PAGE] })
    ],
    [
      FRONT_PAGE,
      block({
    ...
    [
      IMAGE_BLOCK,
      block({ kind: 'image', parent: FRONT_PAGE, fill: IMAGE_FILL })
    ],
    [
      SHAPE_BLOCK,
      block({ kind: 'shape', parent: FRONT_PAGE, fill: SHAPE_FILL })
    ],
    [STICKER_BLOCK, block({ kind: 'sticker', parent: FRONT_PAGE })],
    [
      ACCENT_BLOCK,
      block({
        kind: 'shape',
  ...
  const properties = new Map<string, unknown>([
    ...
    [`${ACCENT_BLOCK}:fill/solid/color`, BLACK],
    [`${BACKGROUND_BLOCK}:fill/solid/color`, BLACK],
    [`${FRONT_PAGE}:fill/solid/color`, BLACK],
    [`${BACK_PAGE}:fill/solid/color`, BLACK],
    [`${TEXT_BLOCK}:text/horizontalAlignment`, 'Left'],
    [`${IMAGE_BLOCK}:placeholder/enabled`, false],
    [`${TEXT_BLOCK}:text/fontSize`, 22],
  ...
  const actionRegistry = new Map<string, (...args: never[]) => unknown>();

  /**
   * The engine ships these six; the kit registers only its own on top, so the
   * fake has to answer them the way the real registry does.
   */
  const registerEngineDefaults = () => {
    const defaults: Record<string, () => void> = {
      'history.undo': () => engine.editor.undo(),
      'history.redo': () => engine.editor.redo(),
      'selection.delete': () =>
        engine.block
          .findAllSelected()
          .forEach((id) => engine.block.destroy(id)),
      'crop.reset': () =>
        engine.block
          .findAllSelected()
          .forEach((id) => engine.block.resetCrop(id)),
      'crop.enter': () => engine.editor.setEditMode('Crop'),
      'editmode.exit': () => engine.editor.setEditMode('Transform')
    };
    for (const [id, run] of Object.entries(defaults)) {
      actionRegistry.set(id, run as (...args: never[]) => unknown);
    }
  };
  ...
  const engine = {
    ...
    actions: {
      register: vi.fn((id: string, run: (...args: never[]) => unknown) => {
        actionRegistry.set(id, run);
      }),
      run: vi.fn((id: string, ...args: never[]) => {
        const run = actionRegistry.get(id);
        if (run == null) throw new Error(`no action ${id} is registered`);
        return run(...args);
      })
    },
    ...
    scene: {
      ...
      get: () => SCENE,
      getPages: vi.fn(() => [FRONT_PAGE, BACK_PAGE]),
      getCurrentPage: vi.fn(() => FRONT_PAGE),
    ...
    block: {
      ...
      supportsStroke: vi.fn((id: number) => get(id).kind === 'shape'),
