...
): Promise<AssetsQueryResult> => (await source.findAssets!(query))!;

afterEach(() => {
  vi.restoreAllMocks();
});

describe('PC-U10 which blocks contribute colours', () => {
  it.each([
  ...
  ])('PC-U10 skips %s', async (_case, block) => {
  ...
  it('PC-U10 keeps a block whose colours cannot be extracted out of the palette', async () => {
...
describe('PC-U11 how a repeated image is recognised', () => {
  it.each([
    [
      'the image file URI',
  ...
  ])('PC-U11 collapses two blocks that share %s', async (_case, a, b) => {
    const source = createImageColorsSource(fakeEngine({ 1: a, 2: b }));

    expect(await source.getGroups!()).toEqual(['Image 1']);
  });

  it('PC-U11 keeps a block whose identity is unknown', async () => {
    const source = createImageColorsSource(
      fakeEngine({
  ...
  it('PC-U11 ignores an empty URI and an empty source set', async () => {
    const source = createImageColorsSource(
      fakeEngine({
...
describe('PC-U12 the group names', () => {
  it('PC-U12 prefers the block name, then its fallback metadata', async () => {
    const source = createImageColorsSource(
      fakeEngine({
  ...
  it('PC-U12 numbers repeated names', async () => {
    const source = createImageColorsSource(
      fakeEngine({
...
describe('PC-U13 querying the palette', () => {
  const engine = () =>
    fakeEngine({
  ...
  it('PC-U13 dedupes the colours of one block', async () => {
    const source = createImageColorsSource(engine());

    const result = await findAssets(source, { page: 0, perPage: 99 });

    expect(result.assets.map((asset: { id: string }) => asset.id)).toEqual([
  ...
  ])('PC-U13 narrows to %s', async (_case, groups, expected) => {
    const source = createImageColorsSource(engine());

    const result = await findAssets(source, {
      page: 0,
      perPage: 99,
  ...
  it('PC-U13 treats an empty group list as no filter', async () => {
    const source = createImageColorsSource(engine());
  ...
  it.each([
    ...
    ['punctuation only', '!!!', 3]
  ])('PC-U13 matches %s', async (_case, query, expected) => {
    const source = createImageColorsSource(engine());

    const result = await findAssets(source, { page: 0, perPage: 99, query });
