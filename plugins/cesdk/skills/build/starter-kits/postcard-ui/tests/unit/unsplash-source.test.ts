...
import type { AssetQueryData, AssetsQueryResult } from '@cesdk/engine';
import { afterEach, describe, expect, it, vi } from 'vitest';
import createUnsplashSource from '@/imgly/plugins/unsplash';
...
/** `AssetSource.findAssets` is optional in the engine's types. */
const findAssets = async (query: AssetQueryData): Promise<AssetsQueryResult> =>
  (await source().findAssets!(query))!;

afterEach(() => {
  vi.unstubAllGlobals();
});

describe('PC-U4 Unsplash response mapping', () => {
  it('PC-U4 maps a photo to an AssetResult', async () => {
  ...
  it('PC-U4 falls back to alt_description, then to undefined', async () => {
    stubFetch({
      results: [
  ...
  it('PC-U4 rejects with the first error message', async () => {
...
describe('PC-U5 Unsplash paging', () => {
  ...
  it('PC-U5 asks Unsplash for page 1 when CE.SDK asks for page 0', async () => {
  ...
  it('PC-U5 asks Unsplash for page 2 when CE.SDK asks for page 1', async () => {
  ...
  it('PC-U5 reports no next page on the last one', async () => {
    const { requested, result } = await searchPage(3);
    expect(requested).toBe('4');
    expect(result.nextPage).toBeUndefined();
  });

  it('PC-U5 pages the unqueried listing the same way', async () => {
    const fetchMock = stubFetch([PHOTO], { total: 25 });
    const result = await findAssets({ page: 1, perPage: 10 });
    const url = new URL(String(fetchMock.mock.calls[0][0]));
...
describe('PC-U14 what Unsplash cannot answer', () => {
  it('PC-U14 rejects a failing listing with its first error', async () => {
  ...
  it('PC-U14 reports no next page once the listing is exhausted', async () => {
    stubFetch([PHOTO], { total: 1 });
    const result = await findAssets({ page: 0, perPage: 10 });
    expect(result.nextPage).toBeUndefined();
  });

  it('PC-U14 offers no groups and refuses to change the library', async () => {
  ...
  it('PC-U14 maps the tags Unsplash sends and omits absent credits', async () => {
    stubFetch({
...
describe('PC-U15 the tracked download', () => {
  const engine = () =>
    ({
      asset: {
  ...
  it('PC-U15 applies the tracked URL to a new block', async () => {
  ...
  it('PC-U15 applies the tracked URL to an existing block', async () => {
    stubFetch({ url: 'https://images.example/full.jpg' });
    const cesdk = engine();
  ...
  it('PC-U15 reports every error the download endpoint returns', async () => {
  ...
  it('PC-U15 refuses an asset with no URI before it calls the endpoint', async () => {
