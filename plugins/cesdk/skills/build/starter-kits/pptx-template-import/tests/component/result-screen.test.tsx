...
describe('PPTX-U6 the archive download name', () => {
  it.each([
    ['example-1-skin.pptx', 'example-1-skin.imgly'],
    ['A.PPTX', 'A.imgly'],
...
describe('the result screen shows both files side by side', () => {
  it('names the two columns and offers Edit, Download and New File', () => {
    renderResult();
    expect(
      screen.getByRole('heading', { name: 'PowerPoint File' })
    ).toBeDefined();
    expect(
  ...
  it('shows the original preview for a pre-loaded example', () => {
    ...
    expect(screen.getByAltText('Original PowerPoint File')).toHaveProperty(
      'src',
      EXAMPLE.previewUrl
    );
  });
  ...
  it('says there is no preview for an uploaded file', () => {
    renderResult('uploaded.pptx', UPLOADED);
    expect(screen.getByText('No Preview Available')).toBeDefined();
    expect(screen.queryByAltText('Original PowerPoint File')).toBeNull();
    expect(screen.getByText(/uploaded\.pptx/)).toBeDefined();
  });
  ...
  it('names the file generically when there is nothing to name', () => {
    renderResult('uploaded.pptx', null);
