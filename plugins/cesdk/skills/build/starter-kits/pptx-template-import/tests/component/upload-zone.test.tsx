...
function renderZone() {
  ...
  return screen.getByLabelText(/Upload PowerPoint File/);
}
...
describe('PPTX-U1 the upload zone filters what it forwards', () => {
  it.each(['a.pptx', 'a.PPTX'])('forwards a dropped %s', (name) => {
  ...
  it.each(['a.ppt', 'a.txt', 'noextension'])('ignores a dropped %s', (name) => {
    const input = renderZone();
    fireEvent.drop(input, { dataTransfer: { files: [file(name)] } });
  ...
  it('keeps the zone highlighted while the pointer moves over it', () => {
    ...
    fireEvent.dragOver(input, { dataTransfer: { files: [file('a.pptx')] } });
  ...
  it('highlights the zone while a file is dragged over it', () => {
    ...
    fireEvent.dragEnter(input, { dataTransfer: { files: [file('a.pptx')] } });
    expect(zone.className).toContain('dragging');

    fireEvent.dragLeave(input, { dataTransfer: { files: [file('a.pptx')] } });
    expect(zone.className).not.toContain('dragging');
  });

  it('forwards a file picked through the dialog and clears the input', () => {
    const input = renderZone() as HTMLInputElement;
    fireEvent.change(input, { target: { files: [file('a.pptx')] } });
    expect(onUpload).toHaveBeenCalledTimes(1);
