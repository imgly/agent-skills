...
test.describe('File selection', () => {
  test('PPTX-01 the kit starts on the selection screen with no engine', async ({
    page
  }) => {
    const app = new KitApp(page);
    await app.open();

    await expect(page.getByText('Supports .pptx Format')).toBeVisible();
  ...
  test('PPTX-04 a dropped file is imported', async ({ page }) => {
    const app = new KitApp(page);
    await app.open();
  ...
  test('PPTX-05 a dropped file of another type is not imported', async ({
    page
  }) => {
    const app = new KitApp(page);
    await app.open();

    await app.drop('not-a-pptx.txt', Buffer.from('x'.repeat(1024)));
  ...
  test('PPTX-05b a rejected drop says why', async ({ page }) => {
    const app = new KitApp(page);
    await app.open();
    ...
    await expect(
      page.getByText(/\.pptx/i).and(page.getByRole('alert'))
    ).toBeVisible();
  });
});
