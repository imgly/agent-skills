...
import { initPptxTemplateImportEditor } from '../../src/imgly';
...
describe('PPTX-U8 initPptxTemplateImportEditor', () => {
  const spy = createApiSpy<CreativeEditorSDK>();
  let added: AddedPlugin[] = [];

  beforeAll(async () => {
    await initPptxTemplateImportEditor(spy.api);
  ...
  it('registers every asset source in one concurrent batch', async () => {
    ...
    const configured = initPptxTemplateImportEditor(cesdk);
...
describe('PPTX-U15 the video timeline the kit ships is inert', () => {
