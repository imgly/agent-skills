...
describe('PRP-U13 setupPanels and setupUI', () => {
...
describe('PRP-U14 setupCanvas and setupInspectorBar', () => {
...
describe('PRP-U15 setupNavigationBar and setupSettings', () => {
  it("ends the navigation bar with the kit's own export button", () => {
    const entries = barFor(
      run<CreativeEditorSDK>(setupNavigationBar),
      'ly.img.navigation.bar'
    );
    expect(entries.at(-1)).toBe('ly.img.export-print-ready-pdf.navigationBar');
    expect(entries).not.toContain('ly.img.exportPDF.navigationBar');
  });
...
describe('PRP-U16 the setups this kit leaves empty', () => {
  it.each([
...
describe('PRP-U17 DesignEditorConfig', () => {
...
describe('PRP-U18 the action handlers the kit registers', () => {
  function handlers() {
