...
const create = vi.fn();
const initPrintReadyPdfEditor = vi.fn(async () => undefined);
const reportDemoPhase = vi.fn();

vi.mock('@cesdk/cesdk-js', () => ({ default: { create } }));
vi.mock('../../src/imgly', () => ({ initPrintReadyPdfEditor }));
...
describe('PRP-U19 src/index.ts', () => {
  beforeEach(() => {
    vi.resetModules();
    create.mockReset();
    initPrintReadyPdfEditor.mockClear();
  ...
  it('creates the editor, initialises the print-ready editor and loads the example scene', async () => {
    ...
    expect(config.userId).toBe('starterkit-print-ready-pdf-editor-user');
    expect(initPrintReadyPdfEditor).toHaveBeenCalledWith(editor);
    expect(editor.load).toHaveBeenCalledWith(
      'http://localhost/assets/example-1.scene'
    );
  ...
  it('reports a failed start-up instead of leaving an unhandled rejection', async () => {
    ...
    expect(initPrintReadyPdfEditor).not.toHaveBeenCalled();
