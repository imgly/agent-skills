...
describe('PSD-U11 the loading screen reports how long the import runs', () => {
  it('shows the message the kit chose for the current step', () => {
    render(<LoadingScreen text="Processing PSD file..." />);
    expect(screen.getByText('Processing PSD file...')).toBeDefined();
  });

  it('starts the stopwatch at zero and counts up', () => {
    const { container } = render(<LoadingScreen text="Loading PSD file..." />);
  ...
  it('shows the previous run time beside the current one', () => {
    const { container } = render(
      <LoadingScreen text="Processing PSD file..." lastInferenceTime={2.5} />
    );
    tick(1000);
  ...
  it('stops counting when the screen goes away', () => {
    const { unmount } = render(<LoadingScreen text="Loading PSD file..." />);
