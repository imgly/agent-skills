...
describe('PSD-U6 the archive download name', () => {
  it.each([
    ['showcase-file-1.psd', 'showcase-file-1.imgly'],
    ['A.PSD', 'A.imgly'],
  ...
  it('turns a.psb into a.imgly', async () => {
    renderResult('a.psb');
    await userEvent.click(
      screen.getByRole('button', { name: /Download CE\.SDK Archive/ })
    );
    expect(clicked[0].download).toBe('a.imgly');
  });
});
...
describe('the result screen shows both files side by side', () => {
  it('names the two columns and offers Edit, Download and New File', () => {
    renderResult();
    expect(
      screen.getByRole('heading', { name: 'Photoshop File' })
    ).toBeDefined();
    expect(
  ...
  it('shows the original preview for a pre-loaded example', () => {
    ...
    expect(screen.getByAltText('Original Photoshop File')).toHaveProperty(
      'src',
      EXAMPLE.previewUrl
    );
  });
  ...
  it('says there is no preview for an uploaded file', () => {
    renderResult('uploaded.psd', UPLOADED);
    expect(screen.getByText('No Preview Available')).toBeDefined();
    expect(screen.queryByAltText('Original Photoshop File')).toBeNull();
    expect(screen.getByText(/uploaded\.psd/)).toBeDefined();
  });
  ...
  it('names the file generically when there is nothing to name', () => {
    renderResult('uploaded.psd', null);
