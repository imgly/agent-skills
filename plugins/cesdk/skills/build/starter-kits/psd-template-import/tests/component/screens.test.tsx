...
describe('PSD-U12 the screen follows the import state', () => {
  it('shows the error screen and retries the same file', async () => {
    mocks.importPsdFile.mockRejectedValueOnce(new Error('broken'));
    mocks.importPsdFile.mockResolvedValue(RESULT);
    renderApp();

    fireEvent.click(screen.getByRole('button', { name: /Skin Care Template/ }));
    ...
    expect(mocks.importPsdFile).toHaveBeenCalledTimes(2);
  });

  it('opens on the selection screen', () => {
    renderApp();
    expect(screen.getByLabelText(/Upload Photoshop File/)).toBeDefined();
  ...
  it('shows the loading screen while the file is being imported', async () => {
    mocks.importPsdFile.mockReturnValue(pending());
    renderApp();
  ...
  it('shows the result screen once the import is done', async () => {
    ...
    expect(screen.queryByText('Processing PSD file...')).toBeNull();
  });
});

describe('PSD-U13 the selection screen offers both ways to pick a file', () => {
  it('imports the example file the user clicked', async () => {
    ...
    await waitFor(() => expect(mocks.importPsdFile).toHaveBeenCalledTimes(1));
    expect(fetch).toHaveBeenCalledWith(
      expect.stringContaining('showcase-file-1.psd')
    );
    expect(mocks.importPsdFile.mock.calls[0][1]).toBe('showcase-file-1');
  });

  it('imports a file dropped on the upload zone, without fetching anything', async () => {
    mocks.importPsdFile.mockResolvedValue(RESULT);
    renderApp();
    const dropped = new File(['bytes'], 'own-design.psd');

    await act(async () => {
      fireEvent.drop(screen.getByLabelText(/Upload Photoshop File/), {
    ...
    expect(mocks.importPsdFile.mock.calls[0][0]).toBe(dropped);
  });
});

describe('PSD-U18 the demo lifecycle beacon', () => {
