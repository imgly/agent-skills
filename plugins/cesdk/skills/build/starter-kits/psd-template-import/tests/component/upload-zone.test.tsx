...
function renderZone() {
  ...
  return screen.getByLabelText(/Upload Photoshop File/);
}
...
describe('PSD-U1 the upload zone filters what it forwards', () => {
  it.each(['a.psd', 'a.PSD', 'a.psb'])('forwards a dropped %s', (name) => {
  ...
  it.each(['a.txt', 'a.pdf', 'noextension'])('ignores a dropped %s', (name) => {
    const input = renderZone();
    fireEvent.drop(input, { dataTransfer: { files: [file(name)] } });
  ...
  it('highlights the zone while a file is dragged over it', () => {
    ...
    fireEvent.dragEnter(input, { dataTransfer: { files: [file('a.psd')] } });
    expect(zone.className).toContain('dragging');

    fireEvent.dragLeave(input, { dataTransfer: { files: [file('a.psd')] } });
    expect(zone.className).not.toContain('dragging');
  });
  ...
  it('offers the accepted extensions to the file dialog', () => {
    const input = renderZone() as HTMLInputElement;
    expect(input.accept).toBe('.psd,.psb');
  });
