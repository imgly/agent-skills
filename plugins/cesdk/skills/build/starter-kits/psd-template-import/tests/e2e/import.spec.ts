...
test.describe('Import', () => {
  test('PSD-02 a file picked in the dialog is imported', async ({ page }) => {
    ...
    await expect(page.getByAltText('Original Photoshop File')).toHaveCount(0);
  });

  test('PSD-03 an example reports loading, then processing, then both previews', async ({
    page
  }) => {
    const app = new KitApp(page);
    await app.open();
    ...
    await page.route('**/*.psd', hold);
    await page.route('**/*.wasm', hold);

    await app.example(EXAMPLES[2]).click();
    await expect(page.getByText('Loading PSD file...')).toBeVisible();
    await expect(page.getByText('Processing PSD file...')).toBeVisible();
    await expect(page.getByText(/^\d+\.\d\ds$/)).toBeVisible();

    await app.waitForResult();
    await expect(page.getByAltText('Original Photoshop File')).toBeVisible();
    await expect(page.getByAltText('Imported Result')).toBeVisible();
  });

  test('PSD-07 the result screen puts the two files side by side', async ({
    page
  }) => {
    const app = new KitApp(page);
    await app.open();
    ...
    await expect(
      page.getByRole('heading', { name: 'Photoshop File' })
    ).toBeVisible();
    await expect(
  ...
  test('PSD-08 a clean import shows no warning and no error badge', async ({
    page
  }) => {
    const app = new KitApp(page);
    await app.open();
  ...
  test('PSD-09 the archive downloads under the imported file name', async ({
    page
  }) => {
    const app = new KitApp(page);
    await app.open();
    ...
    expect(files[0].name).toBe('showcase-file-1.imgly');
    expect(files[0].buffer.subarray(0, 2).toString()).toBe('PK');
  });

  test('PSD-11 New File returns to the selection screen', async ({ page }) => {
    const app = new KitApp(page);
    await app.open();
    await app.importExample(EXAMPLES[2]);
  ...
  test('PSD-06 each of the three examples imports', async ({ page }) => {
    test.slow();
    const app = new KitApp(page);
    await app.open();
