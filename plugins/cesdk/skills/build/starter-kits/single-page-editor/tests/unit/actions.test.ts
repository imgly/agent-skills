...
describe('SPE-U10 the actions the kit registers', () => {
  ...
  it('exportDesign downloads what the editor exported', async () => {
    const { cesdk, run } = registerActions();

    await run('exportDesign', ...([{ mimeType: 'image/png' }] as never[]));

    expect(cesdk.utils.export).toHaveBeenCalledWith({ mimeType: 'image/png' });
    expect(cesdk.utils.downloadFile).toHaveBeenCalledWith(
      expect.any(Blob),
      'image/png'
    );
