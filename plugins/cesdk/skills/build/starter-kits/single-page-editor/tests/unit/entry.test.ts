...
const create = vi.fn();
const initSinglePageEditor = vi.fn(async () => undefined);
const reportDemoPhase = vi.fn();

vi.mock('@cesdk/cesdk-js', () => ({ default: { create } }));
vi.mock('../../src/imgly', () => ({ initSinglePageEditor }));
...
const SCENE = '/assets/ig-post/scene.scene';

describe('SPE-U13 src/index.ts', () => {
  beforeEach(() => {
    vi.resetModules();
    vi.unstubAllEnvs();
    create.mockReset();
    initSinglePageEditor.mockClear();
  ...
  it('loads the scene from the configured base URL', async () => {
    ...
    expect(config.userId).toBe('starterkit-single-page-editor-user');
    expect(initSinglePageEditor).toHaveBeenCalledWith(editor);
  ...
  it('reports a failed start-up instead of leaving an unhandled rejection', async () => {
    ...
    expect(initSinglePageEditor).not.toHaveBeenCalled();
