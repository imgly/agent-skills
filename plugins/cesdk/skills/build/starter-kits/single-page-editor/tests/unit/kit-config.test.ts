...
const INSPECTOR = '//ly.img.panel/inspector';
const ASSETS = '//ly.img.panel/assetLibrary';
...
describe('SPE-U7 the UI layout', () => {
  it('shows large labelled dock icons in the documented order', () => {
    const spy = createApiSpy<CreativeEditorSDK>();
    setupDock(spy.api);
  ...
  it('puts the page-select component between the settings and add-page buttons', () => {
    const spy = createApiSpy<CreativeEditorSDK>();
    setupCanvas(spy.api);

    const [target, order] = spy.callsTo('ui.setComponentOrder')[0].args as [
      { in: string; at: string },
      string[]
    ];
    expect(target).toEqual({ in: 'ly.img.canvas.bar', at: 'bottom' });
  ...
  it('ends the navigation bar with an actions entry holding the two exports', () => {
    const spy = createApiSpy<CreativeEditorSDK>();
  ...
  it('docks the inspector and the asset library on the left without floating them', () => {
    const spy = createApiSpy<CreativeEditorSDK>();
    setupPanels(spy.api);

    const positions = new Map(
      spy.callsTo('ui.setPanelPosition').map(({ args }) => [args[0], args[1]])
    );
    const floating = new Map(
      spy.callsTo('ui.setPanelFloating').map(({ args }) => [args[0], args[1]])
    );

    expect(positions.get(INSPECTOR)).toBe('left');
    expect(positions.get(ASSETS)).toBe('left');
    expect(floating.get(INSPECTOR)).toBe(false);
    expect(floating.get(ASSETS)).toBe(false);
  });
});

describe('SPE-U8 setupActions', () => {
  const spy = createApiSpy<CreativeEditorSDK>();
  ...
  it('forwards the export options and downloads the first blob', async () => {
    const exported: unknown[] = [];
    const downloaded: unknown[][] = [];
    const blob = new Blob(['x'], { type: 'image/png' });
    const handlers = new Map<string, (options: unknown) => Promise<void>>();
    const cesdk = {
      actions: {
        register: (id: string, handler: (options: unknown) => Promise<void>) =>
          handlers.set(id, handler)
      },
      utils: {
        export: async (options: unknown) => {
          exported.push(options);
          return { blobs: [blob], options: { mimeType: 'image/png' } };
        },
    ...
    expect(exported).toEqual([{ mimeType: 'image/png', targetWidth: 512 }]);
    expect(downloaded).toEqual([[blob, 'image/png']]);
  });
});
