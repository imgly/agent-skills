...
describe('TSD-U7 setupNavigationBar', () => {
  const spy = createApiSpy<CreativeEditorSDK>();
  ...
  it('holds undo/redo, the title, zoom and preview', () => {
...
describe('TSD-U7 setupCanvas', () => {
  ...
  it('offers document settings and no page-add button', () => {
    expect(canvasBar?.args[0]).toEqual({
      in: 'ly.img.canvas.bar',
      at: 'bottom'
    });
    // One page per decoration area: a page added from the canvas bar would
    // have no backdrop and no area button.
...
describe('TSD-U7 setupComponents', () => {
  const spy = createApiSpy<CreativeEditorSDK>();
  setupComponents(spy.api);

  it('registers no custom component; the sidebar is plain React', () => {
    expect(spy.callsTo('ui.registerComponent')).toEqual([]);
  });
});

describe('TSD-U7 setupDock', () => {
  const spy = createApiSpy<CreativeEditorSDK>();
  setupDock(spy.api);
