...
describe('TH-U10 setupPanels and setupUI', () => {
...
describe('TH-U11 setupCanvas and setupInspectorBar', () => {
...
describe('TH-U12 setupNavigationBar and setupSettings', () => {
  it('offers undo, zoom and an export entry in the navigation bar', () => {
    const entries = barFor(
      run<CreativeEditorSDK>(setupNavigationBar),
      'ly.img.navigation.bar'
    );
    expect(entries).toContain('ly.img.undoRedo.navigationBar');
    expect(entries).toContain('ly.img.zoom.navigationBar');
    expect(
      entries.some(
        (entry) =>
          typeof entry === 'object' &&
...
describe('TH-U13 the setups this kit leaves empty', () => {
  it.each([
...
describe('TH-U14 DesignEditorConfig', () => {
...
describe('TH-U15 the action handlers the kit registers', () => {
  function handlers() {
