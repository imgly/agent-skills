import { vi } from 'vitest';

// `src/index.ts` reads `window` at module scope and publishes the editor on it.
vi.hoisted(() => {
  const global = globalThis as { window?: unknown };
  global.window ??= globalThis;
  (global.window as { location?: unknown }).location ??= new URL(
    'http://localhost/'
  );
});

import { afterEach, beforeEach, describe, expect, it } from 'vitest';

const create = vi.fn();
const initVideoAnimationsEditor = vi.fn(async () => undefined);
const openAnimationPanel = vi.fn();
const reportDemoPhase = vi.fn();

vi.mock('@cesdk/cesdk-js', () => ({ default: { create } }));
vi.mock('../../src/imgly', () => ({
  DEMO_ASSETS_BASE_URL: 'https://assets.test/video-animations',
  initVideoAnimationsEditor,
  openAnimationPanel
}));
vi.mock('../../../shared/demo-preview/lifecycle', () => ({ reportDemoPhase }));

const onActiveChanged = vi.fn();
const editor = {
  load: vi.fn(async () => undefined),
  engine: { scene: { onActiveChanged } }
};

function phases(): string[] {
  return reportDemoPhase.mock.calls.map(([phase]) => phase as string);
}

describe('VAN-U18 src/index.ts', () => {
  beforeEach(() => {
    vi.resetModules();
    create.mockReset();
    initVideoAnimationsEditor.mockClear();
    openAnimationPanel.mockClear();
    onActiveChanged.mockClear();
    editor.load.mockClear();
    reportDemoPhase.mockClear();
  });

  afterEach(() => {
    delete (globalThis.window as { cesdk?: unknown }).cesdk;
  });

  it('loads the demo template and opens the animation panel on every scene change', async () => {
    create.mockResolvedValue(editor);

    await import('../../src/index');
    await vi.waitFor(() => expect(onActiveChanged).toHaveBeenCalledTimes(1));

    const [container, config] = create.mock.calls[0] as [
      string,
      { userId: string }
    ];
    expect(container).toBe('#cesdk_container');
    expect(config.userId).toBe('starterkit-video-animations-user');
    expect(initVideoAnimationsEditor).toHaveBeenCalledWith(editor);
    expect(editor.load).toHaveBeenCalledWith(
      'https://assets.test/video-animations/assets/templates/lunar-video-default/scene.scene'
    );
    expect(openAnimationPanel).toHaveBeenCalledTimes(1);

    onActiveChanged.mock.calls[0][0]();
    expect(openAnimationPanel).toHaveBeenCalledTimes(2);
    expect((globalThis.window as { cesdk?: unknown }).cesdk).toBe(editor);
  });

  it('reports the demo phases around the editor it built', async () => {
    create.mockResolvedValue(editor);

    await import('../../src/index');
    await vi.waitFor(() => expect(phases()).toEqual(['created', 'ready']));
  });

  it('reports a failed start-up instead of leaving an unhandled rejection', async () => {
    const failure = new Error('no license');
    create.mockRejectedValue(failure);
    const consoleError = vi
      .spyOn(console, 'error')
      .mockImplementation(() => {});

    await import('../../src/index');
    await vi.waitFor(() =>
      expect(consoleError).toHaveBeenCalledWith(
        'Failed to initialize CE.SDK:',
        failure
      )
    );

    expect(initVideoAnimationsEditor).not.toHaveBeenCalled();
    expect(phases()).toEqual(['failed']);
    consoleError.mockRestore();
  });
});
