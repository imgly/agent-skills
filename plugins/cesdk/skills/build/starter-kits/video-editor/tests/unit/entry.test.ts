...
const create = vi.fn();
const initVideoEditor = vi.fn(async () => undefined);
const reportDemoPhase = vi.fn();

vi.mock('@cesdk/cesdk-js', () => ({ default: { create } }));
vi.mock('../../src/imgly', () => ({ initVideoEditor }));
...
describe('VED-U6 src/index.ts', () => {
  beforeEach(() => {
    vi.resetModules();
    vi.unstubAllEnvs();
    create.mockReset();
    initVideoEditor.mockClear();
  ...
  it('loads the scene from the configured base URL', async () => {
    ...
    expect(config.userId).toBe('starterkit-video-editor-user');
    expect(initVideoEditor).toHaveBeenCalledWith(editor);
  ...
  it('reports a failed start-up instead of leaving an unhandled rejection', async () => {
    ...
    expect(initVideoEditor).not.toHaveBeenCalled();
