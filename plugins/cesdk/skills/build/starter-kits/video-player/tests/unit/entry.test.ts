...
const create = vi.fn();
const initVideoPlayer = vi.fn(async () => undefined);

vi.mock('@cesdk/cesdk-js', () => ({ default: { create } }));
vi.mock('../../src/imgly', () => ({ initVideoPlayer }));
...
const SCENE = '/assets/video-fashion-portfolio/scene.scene';

describe('VPY-U3 src/index.ts', () => {
  beforeEach(() => {
    vi.resetModules();
    vi.unstubAllEnvs();
    create.mockReset();
    initVideoPlayer.mockClear();
  ...
  it('loads the scene from the configured base URL', async () => {
    ...
    expect(config.userId).toBe('starterkit-video-player-user');
    expect(initVideoPlayer).toHaveBeenCalledWith(editor);
  ...
  it('reports a failed start-up instead of leaving an unhandled rejection', async () => {
    ...
    expect(initVideoPlayer).not.toHaveBeenCalled();
