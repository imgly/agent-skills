...
Design editing with advanced export options—choose format, quality, resolution, and page range for exports. Runs entirely in the browser with no server dependencies.
...
<Tabs syncKey="project-type">
  ...
  <TabItem label="Existing Project">
    ...
    Integrate the Export Options Editor into an existing Angular application. This adds the editor configuration to your current project structure.
    ...
    <TerminalTabs>
      <TerminalTab label="git">
        git clone https://github.com/imgly/starterkit-export-options-ts-web.git
        cp -r starterkit-export-options-ts-web/src/app/imgly ./src/app/imgly
        rm -rf starterkit-export-options-ts-web
      </TerminalTab>

      <TerminalTab label="degit">
        npx degit imgly/starterkit-export-options-ts-web/src/app/imgly ./src/app/imgly
      </TerminalTab>
    </TerminalTabs>
    ...
    export class DesignEditorComponent {
      initExportOptionsEditor = initExportOptionsEditor;
    }
    ```
...
```typescript title="src/app/imgly/plugins/export-design-panel.ts"
