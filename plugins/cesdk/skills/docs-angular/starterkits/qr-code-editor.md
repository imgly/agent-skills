...
QR code generation for your Angular app—create customizable QR codes with a single click. Runs entirely in the browser with no server dependencies.
...
<Tabs syncKey="project-type">
  <TabItem label="New Project">
    ...
    Create a new Angular application with QR Code Editor integration.
    ...
    ```bash
    ng generate component qr-code-editor
    ```

    Update `qr-code-editor.component.ts`:
    ...
    @Component({
      selector: 'app-qr-code-editor',
      standalone: true,
      template: '<div #container style="width: 100%; height: 100vh;"></div>',
    })
    export class QRCodeEditorComponent implements OnInit, OnDestroy {
      ...
      async ngOnInit() {
        ...
        await initQRCodeEditor(this.cesdk);
      }
    ...
    ```typescript
    import { QRCodeEditorComponent } from './qr-code-editor/qr-code-editor.component';

    @Component({
      selector: 'app-root',
      standalone: true,
      imports: [QRCodeEditorComponent],
      template: '<app-qr-code-editor></app-qr-code-editor>',
    })
  ...
  <TabItem label="Existing Project">
    ...
    Integrate the QR Code Editor into an existing Angular application.
