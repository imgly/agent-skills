...
```ts
type PageAddAction = (options?) => Promise<number>;
```

Action function for adding a page
...
| `options.index?` | `number` | Child index in the page container to insert at (defaults to appending) |
