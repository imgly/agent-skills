...
Export videos with GPU-accelerated rendering using the CE.SDK Renderer API. This starterkit demonstrates server-side video export with real-time upload and download progress tracking.
...
<Tabs syncKey="project-type">
  <TabItem label="New Project">
    ...
    Start fresh with a standalone Export Using Renderer project. This creates a complete, ready-to-run application.
    ...
    <TerminalTabs>
      ...
      <TerminalTab label="degit">
        npx degit imgly/starterkit-export-using-renderer-ts-web starterkit-export-using-renderer-ts-web
      </TerminalTab>
    </TerminalTabs>
  ...
  <TabItem label="Existing Project">
    ...
    Integrate the Export Using Renderer into an existing web application. This adds the editor configuration to your current project structure.
...
The Export Using Renderer starterkit includes built-in support for exporting videos via the CE.SDK Renderer API. This enables GPU-accelerated server-side rendering with real-time progress tracking.

### Export Flow

1. **Archive Scene**: The current scene is saved as a compressed archive
2. **Upload**: The archive is sent to the renderer server with progress tracking
3. **Server Render**: The GPU server renders the video
4. **Download**: The rendered video is downloaded with progress tracking
5. **Save**: The video is automatically saved to the user's device

### Configure the Renderer URL

Set the `VITE_RENDERER_PROXY_URL` environment variable to use your own renderer:

```bash
VITE_RENDERER_PROXY_URL=https://your-renderer.example.com/export npm run dev
```
...
The Export Using Renderer starterkit includes everything needed for server-side video export.
...
<CapabilityGrid
  features={[
  {
    ...
    description:
      'GPU-accelerated video export via the CE.SDK Renderer API with progress tracking.',
    imageId: 'transform',
  },
  {
  ...
  },
  {
    title: 'Video Editing',
    description:
      'Edit video timelines with trimming, transitions, and multi-track audio support.',
    imageId: 'transform',
  },
  {
  ...
  },
  {
    ...
    description:
      'Real-time upload and download progress tracking for server-side rendering exports.',
    imageId: 'client-side',
  },
]}
/>
