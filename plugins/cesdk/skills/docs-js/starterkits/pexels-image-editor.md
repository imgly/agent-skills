...
A design editor with Pexels stock photos integrated as the primary image source—search and browse millions of free, high-quality images directly within your editor. Runs entirely in the browser with no server dependencies.

![Pexels Image Editor starter kit showing stock photo integration](https://img.ly/docs/cesdk/./assets/browser.hero.webp)
...
- **Pexels API Proxy Server** – Set up a proxy to securely handle Pexels API authentication. See [Pexels API Documentation](https://www.pexels.com/api/documentation/) for API details.
...
<Tabs syncKey="project-type">
  <TabItem label="New Project">
    ...
    <TerminalTabs>
      ...
      <TerminalTab label="degit">
        npx degit imgly/starterkit-pexels-asset-source-ts-web my-project
        cd my-project
      </TerminalTab>
    </TerminalTabs>

    ## Step 2: Configure Pexels Proxy URL
    ...
    <TerminalTabs>
      <TerminalTab label="bash">
        ...
        # Edit .env and add your VITE\_PEXELS\_PROXY\_URL
      </TerminalTab>
    </TerminalTabs>

    The proxy server handles Pexels API authentication server-side, keeping your API key secure. See [Pexels API Documentation](https://www.pexels.com/api/documentation/) for API details.
    ...
    Open `http://localhost:5173` in your browser. Click "Pexels" in the dock to search and browse stock photos.
  </TabItem>
  ...
  <TabItem label="Existing Project">
    ...
    ## Step 3: Configure Pexels Proxy URL
    ...
    **Environment Variable (Recommended)**
    ...
    ```bash
    # In your .env file
    VITE_PEXELS_PROXY_URL=https://your-proxy-server.com/pexels-api
    ```
    ...
    **Programmatically**
    ...
    await initPexelsImageEditor(cesdk, {
      pexelsProxyUrl: 'https://your-proxy-server.com/pexels-api'
    });
    ```

    The proxy server handles Pexels API authentication server-side, keeping your API key secure and preventing exposure in browser network requests.
    ...
    // Set up a proxy server to handle Pexels API authentication
    const PEXELS_PROXY_URL = 'https://your-proxy-server.com/pexels-api';
    ...
    CreativeEditorSDK.create('#cesdk_container', config)
      .then(async (cesdk) => {
        await initPexelsImageEditor(cesdk, { pexelsProxyUrl: PEXELS_PROXY_URL });
      })
      .catch((error) => {
...
2. The Pexels library opens showing curated photos
3. Click any photo to add it to your design

### Searching Photos

1. Open the Pexels library
...
3. The Pexels library opens automatically
4. Select a new photo to replace the current one

> **Pexels License:** All photos from Pexels are free to use under the [Pexels License](https://www.pexels.com/license/). Attribution is appreciated but not required.
...
```typescript title="src/imgly/plugins/pexels.ts"
export class PexelsAssetSourcePlugin implements EditorPlugin {
  ...
  async initialize({ cesdk }: EditorPluginContext): Promise<void> {
    ...
    const pexelsAssetSource = createPexelsAssetSource(proxyUrl);
...
- **Set your proxy URL**: Add `VITE_PEXELS_PROXY_URL` to your `.env` file or pass it to `initPexelsImageEditor()`
- **Set up a proxy server**: Create a server-side endpoint that handles Pexels API authentication and returns data in CE.SDK format
