...
Professional print-ready PDF export for your web app—create designs with PDF/X-3 compliance, CMYK color profiles, and configurable bleed margins. Runs entirely in the browser with no server dependencies.
...
Click the "Export PDF" button in the navigation bar to open the export panel:
...
cesdk.ui.registerPanel('ly.img.export-print-ready-pdf.panel', ({ builder }) => {
  ...
  builder.Section('bleed-section', { children: () => {
    builder.Checkbox('include-bleed', {
      label: 'Include Bleed',
      value: includeBleed,
      setValue: setIncludeBleed
    });
    // Bleed margin input (in mm)
    builder.NumberInput('bleed-margin', {
      label: 'Bleed Margin (mm)',
      value: bleedMargin,
      setValue: setBleedMargin
    });
  }});
  ...
  builder.Select('color-profile', {
    label: 'Color Profile',
    options: [
      { label: 'ISO Coated v2 (CMYK)', value: 'fogra39' },
      { label: 'GRACoL 2006 (CMYK)', value: 'gracol' },
      { label: 'sRGB (RGB)', value: 'srgb' }
    ]
  });

  // Page range selection
  builder.Select('pages', {
    label: 'Pages',
    options: [
...
You can also trigger PDF export programmatically:

```typescript
// Export with bleed margins and CMYK color profile
const pdfBlob = await cesdk.engine.block.exportPDF(
  pages,
  'application/pdf',
  {
