...
Image vectorization for your web app—convert raster images to scalable vectors with a single click. Runs entirely in the browser with no server dependencies.
...
<Tabs syncKey="project-type">
  <TabItem label="New Project">
    ...
    Run the Vectorizer Editor starter kit directly with Vite.
    ...
    <TerminalTabs>
      ...
      <TerminalTab label="degit">
        npx degit imgly/starterkit-vectorizer-editor-ts-web my-project
        cd my-project
      </TerminalTab>
    </TerminalTabs>
    ...
    Open `http://localhost:5173` in your browser. Select an image and click "Vectorize" in the canvas menu to convert it to vectors.
  </TabItem>

  <TabItem label="Existing Project">
    ## Get Started

    Integrate the Vectorizer Editor into an existing vanilla JavaScript/TypeScript project. This adds the editor configuration to your current project structure.
...
The editor provides vectorization via the canvas menu:
...
4. The image is converted to a scalable vector graphic

The vectorizer analyzes the image and creates clean vector paths that maintain quality at any scale.
...
The Vectorizer Editor includes image vectorization plus full design editing capabilities.
...
<CapabilityGrid
  features={[
  {
    ...
    description:
      'Convert raster images to scalable vector graphics. The AI-powered vectorizer creates clean paths that scale perfectly at any size.',
    imageId: 'transform',
  },
  {
...
### Vectorize option doesn't appear
...
- **Select an image**: The Vectorize option only appears when an image block is selected
