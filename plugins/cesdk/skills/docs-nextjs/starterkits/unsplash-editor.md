...
Design editor with integrated Unsplash image library for your Next.js app. Search and use millions of free high-quality photos with automatic photographer attribution.
...
<Tabs syncKey="project-type">
  <TabItem label="New Project">
    ...
    const UnsplashEditor = dynamic(
      () => import('./components/UnsplashEditor'),
      { ssr: false }
    );
...
```typescript title="app/api/unsplash/route.ts"
// Next.js API Route
...
export async function GET(request: NextRequest) {
  ...
  const response = await fetch(`https://api.unsplash.com/photos?${searchParams}`, {
    headers: {
...
NEXT_PUBLIC_UNSPLASH_API_URL=https://your-domain.com/api/unsplash
```
...
await initUnsplashEditor(cesdk, {
  unsplashApiUrl: process.env.NEXT_PUBLIC_UNSPLASH_API_URL
});
```
...
- **Set environment variable**: Set `NEXT_PUBLIC_UNSPLASH_API_URL` in your `.env.local` file
