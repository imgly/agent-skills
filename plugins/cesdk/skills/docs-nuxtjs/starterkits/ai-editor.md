...
AI-powered creative editing for your Nuxt.js app—generate images, videos, audio,
and voiceovers directly in the editor. Built with React and runs entirely in the browser with
...
<Tabs syncKey="project-type">
  ...
  <TabItem label="Existing Project">
    ...
    Integrate the AI Editor into an existing Nuxt.js application. This adds the editor configuration to your current project structure.
    ...
    <TerminalTabs>
      <TerminalTab label="git">
        git clone https://github.com/imgly/starterkit-ai-editor-react-web.git
        cp -r starterkit-ai-editor-react-web/app/imgly ./app/imgly
        rm -rf starterkit-ai-editor-react-web
      </TerminalTab>

      <TerminalTab label="degit">
        npx degit imgly/starterkit-ai-editor-react-web/app/imgly ./app/imgly
      </TerminalTab>
    </TerminalTabs>
    ...
    <Tabs syncKey="editor-mode">
      <TabItem label="Design">
        ...
        const init = (cesdk: any) => initAiDesignEditor(cesdk, providers);
        </script>
        ```
      </TabItem>
      ...
      <TabItem label="Photo">
        ...
        const init = (cesdk: any) => initAiPhotoEditor(cesdk, providers);
        </script>
        ```
      </TabItem>
      ...
      <TabItem label="Video">
        ...
        const init = (cesdk: any) => initAiVideoEditor(cesdk, providers);
        </script>
        ```
      </TabItem>
    </Tabs>
...
```typescript title="app/imgly/plugins/ai-app/ai-providers.ts"
...
```typescript title="app/imgly/config/index.ts"
...
```typescript title="app/imgly/config/design-editor/plugin.ts"
import {
  FiltersAssetSource,
  StickerAssetSource,
  TextAssetSource,
...
```typescript title="app/imgly/config/design-editor/actions.ts"
...
```typescript title="app/imgly/config/design-editor/settings.ts"
...
```typescript title="app/imgly/config/design-editor/i18n.ts"
...
```typescript title="app/imgly/config/design-editor/features.ts"
...
```typescript title="app/imgly/plugins/ai-app/ai-apps.ts"
...
```typescript title="app/imgly/plugins/background-removal.ts"
