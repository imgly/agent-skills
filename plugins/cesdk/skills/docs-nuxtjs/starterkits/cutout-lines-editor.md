...
Print-ready cutout line creation for your Nuxt.js app—create customizable cutout lines with a single click. Runs entirely in the browser with no server dependencies.
...
<Tabs syncKey="project-type">
  <TabItem label="New Project">
    ...
    Create a new Nuxt.js application with Cutout Lines Editor integration.
    ...
    <TerminalTabs>
      <TerminalTab label="git">
        ...
        cp -r starterkit-cutout-lines-editor-ts-web/src/imgly ./imgly
        rm -rf starterkit-cutout-lines-editor-ts-web
      </TerminalTab>

      <TerminalTab label="degit">
        npx degit imgly/starterkit-cutout-lines-editor-ts-web/src/imgly ./imgly
      </TerminalTab>
    </TerminalTabs>
    ...
    Create `components/CutoutLinesEditor.vue`:
    ...
    onMounted(async () => {
      if (import.meta.client && containerRef.value) {
        ...
        const { initCutoutLinesEditor } = await import('../imgly');
    ...
    > **Client-Only Rendering:** The `ClientOnly` component and `import.meta.client` check ensure CE.SDK only loads in the browser, avoiding SSR issues.

    ## Step 6: Use the Component

    Use the component in a page (e.g., `pages/index.vue`):

    ```vue
    <script setup lang="ts">
    import CutoutLinesEditor from '~/components/CutoutLinesEditor.vue';
    </script>
  ...
  <TabItem label="Existing Project">
    ...
    Integrate the Cutout Lines Editor into an existing Nuxt.js application.
...
CE.SDK requires browser APIs. Use `ClientOnly` component and `import.meta.client` check to ensure client-only loading.
