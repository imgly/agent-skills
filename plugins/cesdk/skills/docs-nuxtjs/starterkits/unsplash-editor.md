...
Design editor with integrated Unsplash image library for your Nuxt.js app. Search and use millions of free high-quality photos with automatic photographer attribution.
...
<Tabs syncKey="project-type">
  <TabItem label="New Project">
    ...
    Create a new Nuxt.js application with Unsplash Editor integration.
    ...
    <TerminalTabs>
      ...
      <TerminalTab label="degit">
        npx degit imgly/starterkit-unsplash-asset-source-ts-web/app/imgly ./app/imgly
      </TerminalTab>
    </TerminalTabs>
    ...
    Create a client-only Vue component using the `.client.vue` suffix (e.g., `UnsplashEditor.client.vue`):
    ...
    Save this file as `components/UnsplashEditor.client.vue` to ensure it only renders on the client.
    ...
    <script setup lang="ts">
    import UnsplashEditor from '~/components/UnsplashEditor.client.vue';
    </script>
    ```
  ...
  <TabItem label="Existing Project">
    ...
    Integrate the Unsplash Editor into an existing Nuxt.js application. This adds the editor configuration to your current project structure.
...
```typescript title="server/api/unsplash.ts"
// Nuxt.js Server API Route
export default defineEventHandler(async (event) => {
  const apiKey = process.env.UNSPLASH_ACCESS_KEY;
  const query = getQuery(event);

  const response = await fetch(`https://api.unsplash.com/photos?${new URLSearchParams(query)}`, {
    headers: {
...
```bash title=".env"
NUXT_PUBLIC_UNSPLASH_API_URL=https://your-domain.com/api/unsplash
```

Then pass it to the init function:

```typescript title="app.vue"
await initUnsplashEditor(cesdk, {
  unsplashApiUrl: useRuntimeConfig().public.unsplashApiUrl
});
```
...
- **Set environment variable**: Set `NUXT_PUBLIC_UNSPLASH_API_URL` in your `.env` file
