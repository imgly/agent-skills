...
> **Tip:** The IMG.LY CDN is intended for development and evaluation. For production,
> self-host the assets and point your `baseURL` at your own location.
...
when using the engine package).

### How it impacts you

Most shortcuts definitions moved out of the SDK internals to the configuration level. While we still ship some defaults, to get the full experince we strongly recommend you take
a moment to integrate our public catalogs into your application. Each starterkit now ships with a complete shortcut catalog adjusted to its use case. See the design and video editor below as an example:

- [Design editor keyboard configuration](https://github.com/imgly/starterkit-design-editor-ts-web/tree/main/src/imgly/config/keyboard)
- [Video editor keyboard configuration](https://github.com/imgly/starterkit-video-editor-ts-web/tree/main/src/imgly/config/keyboard)

### Integrate the full catalog

Copy the `src/imgly/config/keyboard/` folder from the starter kit that matches your editor into
your project. It contains two files:

- `catalogs/us-ansi.ts` — the full US-ANSI shortcut catalog as a plain `KeyboardShortcut[]` array.
- `keyboard.ts` — a thin wrapper that registers the catalog through `cesdk.shortcuts.set(...)`.

Then register the catalog when you initialize the editor:

```ts
import { setupKeyboardShortcuts } from './imgly/config/keyboard/keyboard';
