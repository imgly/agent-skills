...
Getty Images stock photo integration for your SvelteKit app—search and browse millions of premium stock photos from Getty Images directly within your editor. Requires a proxy server for authentication.
...
- **Getty Images Proxy Server** – Set up a proxy server to handle Getty Images authentication
...
<Tabs syncKey="project-type">
  <TabItem label="New Project">
    ...
    Create a new SvelteKit application with Getty Images Editor integration.
    ...
    <TerminalTabs>
      <TerminalTab label="git">
        ...
        cp -r starterkit-getty-asset-source-ts-web/src/lib/imgly ./src/lib/imgly
        cp starterkit-getty-asset-source-ts-web/static/assets/remove-bg.png ./static/assets/
        rm -rf starterkit-getty-asset-source-ts-web
      </TerminalTab>

      <TerminalTab label="degit">
        npx degit imgly/starterkit-getty-asset-source-ts-web/src/lib/imgly ./src/lib/imgly
        npx degit imgly/starterkit-getty-asset-source-ts-web/static/assets ./static/assets
      </TerminalTab>
    </TerminalTabs>
    ...
    Create a Svelte component using the CE.SDK imperative API (e.g., `src/lib/GettyImagesEditor.svelte`):
    ...
    <script>
      ...
      const GETTY_IMAGES_PROXY_URL = 'YOUR_GETTY_IMAGES_PROXY_URL';
      ...
      onMount(async () => {
        ...
        await initGettyImagesEditor(cesdk, { gettyProxyUrl: GETTY_IMAGES_PROXY_URL });
      });
    ...
    <script>
      ...
      let GettyImagesEditor;

      onMount(async () => {
        const module = await import('$lib/GettyImagesEditor.svelte');
        GettyImagesEditor = module.default;
      });
    </script>

    {#if browser && GettyImagesEditor}
      <svelte:component this={GettyImagesEditor} />
    {/if}
    ```
    ...
    ```svelte
    <script>
      import GettyImagesEditor from '$lib/GettyImagesEditor.svelte';
    </script>
  ...
  <TabItem label="Existing Project">
    ...
    Integrate the Getty Images Editor into an existing SvelteKit application.
...
> **Proxy Server Required:** To use Getty Images in production, you need to set up a proxy server for authentication.
> Configure it via the `VITE_GETTY_IMAGES_PROXY_URL` environment variable or pass it to `initGettyImagesEditor()`.
...
<CapabilityGrid
  ...
  },
  {
    ...
    description:
      'Authentication is handled via a proxy server, keeping API credentials secure and away from client code.',
    imageId: 'client-side',
  },
  {
...
- **Check your proxy server**: Ensure your proxy server is running and accessible at the URL configured in `VITE_GETTY_IMAGES_PROXY_URL`
- **Check network requests**: Open DevTools Network tab and look for failed requests to your proxy server
- **Verify configuration**: Ensure the proxy server URL is correctly set via the environment variable or passed to `initGettyImagesEditor()`
