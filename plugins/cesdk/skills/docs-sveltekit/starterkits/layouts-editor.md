...
Professional layout-based editing for your SvelteKit app—create graphics, templates, and
...
<Tabs syncKey="project-type">
  <TabItem label="New Project">
    ...
    Create a new SvelteKit application with Layouts Editor integration.
    ...
    <TerminalTabs>
      <TerminalTab label="git">
        ...
        cp -r starterkit-layouts-asset-source-ts-web/src/lib/imgly ./src/lib/imgly
        rm -rf starterkit-layouts-asset-source-ts-web
      </TerminalTab>

      <TerminalTab label="degit">
        npx degit imgly/starterkit-layouts-asset-source-ts-web/src/lib/imgly ./src/lib/imgly
      </TerminalTab>
    </TerminalTabs>
    ...
    Create a Svelte component using the CE.SDK imperative API (e.g., `src/lib/LayoutsEditor.svelte`):
    ...
    Import and use the Layouts Editor component in your page route (e.g., `src/routes/+page.svelte`):
    ...
    <script>
      ...
      onMount(async () => {
        const module = await import('$lib/LayoutsEditor.svelte');
        LayoutsEditor = module.default;
      });
    </script>

    {#if browser && LayoutsEditor}
      <svelte:component this={LayoutsEditor} />
    {/if}
    ```
    ...
    ```svelte
    <script>
      import LayoutsEditor from '$lib/LayoutsEditor.svelte';
    </script>
  ...
  <TabItem label="Existing Project">
    ...
    Integrate the Layouts Editor into an existing SvelteKit application. This adds the editor configuration to your current project structure.
...
The Layouts Editor includes a custom layouts plugin that allows users to choose from pre-designed layout templates and apply them to existing pages while preserving content. The plugin is implemented in `src/lib/imgly/plugins/layouts/layout.ts`.
...
```typescript title="src/lib/imgly/plugins/layouts/layout.ts"
