...
Design editor with integrated Unsplash image library for your SvelteKit app. Search and use millions of free high-quality photos with automatic photographer attribution.
...
<Tabs syncKey="project-type">
  <TabItem label="New Project">
    ...
    Create a new SvelteKit application with Unsplash Editor integration.
    ...
    <TerminalTabs>
      <TerminalTab label="git">
        ...
        cp -r starterkit-unsplash-asset-source-ts-web/src/lib/imgly ./src/lib/imgly
        rm -rf starterkit-unsplash-asset-source-ts-web
      </TerminalTab>

      <TerminalTab label="degit">
        npx degit imgly/starterkit-unsplash-asset-source-ts-web/src/lib/imgly ./src/lib/imgly
      </TerminalTab>
    </TerminalTabs>
    ...
    Create a Svelte component using the CE.SDK imperative API (e.g., `src/lib/UnsplashEditor.svelte`):
    ...
    Import and use the Unsplash Editor component in your page route (e.g., `src/routes/+page.svelte`):
    ...
    <script>
      ...
      onMount(async () => {
        const module = await import('$lib/UnsplashEditor.svelte');
        UnsplashEditor = module.default;
      });
    </script>

    {#if browser && UnsplashEditor}
      <svelte:component this={UnsplashEditor} />
    {/if}
    ```
    ...
    ```svelte
    <script>
      import UnsplashEditor from '$lib/UnsplashEditor.svelte';
    </script>
  ...
  <TabItem label="Existing Project">
    ...
    Integrate the Unsplash Editor into an existing SvelteKit application. This adds the editor configuration to your current project structure.
...
```typescript title="src/routes/api/unsplash/+server.ts"
// SvelteKit API Route
import { json } from '@sveltejs/kit';
import type { RequestHandler } from './$types';

export const GET: RequestHandler = async ({ url }) => {
  const apiKey = import.meta.env.UNSPLASH_ACCESS_KEY;

  const response = await fetch(`https://api.unsplash.com/photos?${url.searchParams}`, {
    headers: {
...
```typescript title="src/routes/+page.svelte"
await initUnsplashEditor(cesdk, {
