...
Please note, that changes to the asset library entries are now done by the [Asset Library Entry API](./import-media/asset-library/basics.md) and the dock order is just referring to these. So if you, for instance, want to add an asset source id to be shown in a library, you have to add this asset source id to the asset library entry and not to the dock order.
