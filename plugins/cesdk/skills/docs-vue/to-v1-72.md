...
The key change is that scene creation no longer sets a mode automatically. To preserve the old behavior, set the mode explicitly.
...
If your Web integration branches on mode for UI decisions or feature gating, call `setMode()` after `create()` and your existing code continues to work unchanged. Mode no longer affects engine behavior, but your own code can still read and branch on it.
...
|--------|--------|-----------|-------------------|
| Scene mode deprecated | Mode no longer affects engine behavior | Remove mode checks | Call `setMode()` or pass `mode` to action |
